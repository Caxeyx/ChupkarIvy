...
ChupkarIvy is built using [rust](https://www.rust-lang.org/), so you'll need to install that first. It is cross-platform, so it should work on Windows, Linux and MacOS. You can find more info about how to install rust [here](https://www.rust-lang.org/tools/install).

### Rust formatter

ChupkarIvy uses [rustfmt](https://github.com/rust-lang/rustfmt) to format the code, and we ask everyone that contributes to ChupkarIvy to use it as well. You can install it by running the following command in your terminal:
...
Now that you have all the dependencies installed, you can compile ChupkarIvy. To do this, you'll first need to clone the repository:

```sh
git clone https://github.com/Caxeyx/ChupkarIvy.git
```
...
```sh
./target/release/chupkarivy
```

If you are actively developing ChupkarIvy, you can use the following command to build and run the bot (this is easier than building and running the bot manually):
...
As of now, ChupkarIvy has one optional feature: `stats`. This feature enables collecting a few statistics, total and active servers. These statistics will be sent to a redis server, where they then can be read for whatever purpose. If you want to enable this feature, you can do so by running the following command:
